...
export function getUsers() {
  ...
  if (process.env.AUTH_USER_PETER && process.env.AUTH_PASS_PETER) {
    users.push({
      id: 'peter',
      username: process.env.AUTH_USER_PETER.trim(),
      password: process.env.AUTH_PASS_PETER.trim(),
      displayName: (process.env.AUTH_DISPLAY_PETER || process.env.AUTH_USER_PETER).trim(),
    });
  }
...
export function getAuthDiagnostics() {
  ...
  const peterPassRaw = process.env.AUTH_PASS_PETER;
  ...
  return {
    ...
    peter: {
      userConfigured: Boolean(process.env.AUTH_USER_PETER?.trim()),
      passConfigured: Boolean(peterPassRaw),
      passRawLength: peterPassRaw?.length ?? 0,
      passTrimmedLength: peterPassRaw?.trim().length ?? 0,
      passHadWhitespace: peterPassRaw ? peterPassRaw !== peterPassRaw.trim() : false,
      username: process.env.AUTH_USER_PETER?.trim() || null,
    },
  };
}
